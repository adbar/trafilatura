...
- Extraction
   ...
   * ``MIN_OUTPUT_COMM_SIZE`` works the same for comment extraction
...
With the ``--config-file`` option, followed by the file name or path. Values missing from the custom file fall back to the defaults.
