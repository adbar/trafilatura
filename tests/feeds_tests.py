...
def test_rss_extraction():
    ...
    assert len(feeds.extract_links(XMLDECL + '<link>\r\n    https://www.ak-kurier.de/akkurier/www/artikel/108815-sinfonisches-blasorchester-spielt-1500-euro-fuer-kinder-in-drk-krankenhaus-kirchen-ein    </link>', params)) == 1
...
def test_json_extraction():
    ...
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" type="application/json" title="JSON Feed" href="https://www.jsonfeed.org/feed.json" />></meta><body/></html>', params)) == 1
    ...
    # id as a backup
    params = feeds.FeedParameters('https://example.org', 'example.org', '')
    links = feeds.extract_links(r'{"version":"https:\/\/jsonfeed.org\/version\/1","items":[{"id":"https://www.example.org/1","title":"Test"}]}', params)
    assert len(links) == 1
...
def test_feeds_helpers():
    ...
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" type="application/rss+xml" title="Feed"/></meta><body/></html>', params)) == 0
    # useful
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" type="application/rss+xml" title="Feed" href="https://example.org/blog/feed/"/></meta><body/></html>', params)) == 1
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" type="application/atom+xml" title="Feed" href="https://example.org/blog/feed/"/></meta><body/></html>', params)) == 1
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" title="Feed" href="https://example.org/blog/feed/" type="application/atom+xml"/></meta><body/></html>', params)) == 1
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" title="Feed" href="https://example.org/blog/atom/"/></meta><body/></html>', params)) == 1
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" href="https://www.theguardian.com/international/rss" title="RSS" type="application/rss+xml"></meta><body/></html>', params)) == 1
    # no comments wanted
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" type="application/rss+xml" title="Feed" href="https://example.org/blog/comments-feed/"/></meta><body/></html>', params)) == 0

    # invalid links
    params = feeds.FeedParameters('example.org', 'example.org', 'https://example.org')  # fix
    assert len(feeds.determine_feed('<html><meta><link rel="alternate" href="12345tralala" title="RSS" type="application/rss+xml"></meta><body/></html>', params)) == 0

    # detecting in <a>-elements
    params = feeds.FeedParameters('https://example.org', 'example.org', 'https://example.org')
    assert feeds.determine_feed('<html><body><a href="https://example.org/feed.xml"><body/></html>', params) == ['https://example.org/feed.xml']
    assert feeds.determine_feed('<html><body><a href="https://example.org/feed.atom"><body/></html>', params) == ['https://example.org/feed.atom']
    assert feeds.determine_feed('<html><body><a href="https://example.org/rss"><body/></html>', params) == ['https://example.org/rss']
    # feed discovery
    assert not feeds.find_feed_urls('http://')
    assert not feeds.find_feed_urls('https://httpbun.org/status/404')
    # Feedburner/Google links
    assert feeds.handle_link_list(['https://feedproxy.google.com/ABCD'], params) == ['https://feedproxy.google.com/ABCD']
    # override failed checks
    assert feeds.handle_link_list(['https://feedburner.com/kat/1'], params) == ['https://feedburner.com/kat/1']
