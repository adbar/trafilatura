...
def examine(htmlstring, url=None, no_fallback=False, include_comments=True, include_tables=True, csv_output=False, xml_output=False, tei_output=False, validation=False):
    ...
    # proceed
        ...
        result = extract(htmlstring, url, '0000', no_fallback=no_fallback, include_comments=include_comments, include_tables=include_tables, csv_output=csv_output, xml_output=xml_output, tei_output=tei_output, tei_validation=validation)
        return result
    return None
...
def main():
    ...
    argsparser.add_argument("--csv", help="CSV output", action="store_true")
    ...
    if args.inputfile:
        with open(args.inputfile, mode='r', encoding='utf-8') as inputfile: # errors='strict', buffering=1
            for line in inputfile:
                ...
                try:
                    result = examine(htmlstring, url=url, no_fallback=args.fast, include_comments=args.nocomments, include_tables=args.notables, csv_output=args.csv, xml_output=args.xml, tei_output=args.xmltei, validation=args.validate)
                    if result is None:
        ...
        # process input on STDIN
            ...
            try:
                ...
                sys.exit(1)
        # process
        result = examine(htmlstring, url=args.URL, no_fallback=args.fast, include_comments=args.nocomments, include_tables=args.notables, csv_output=args.csv, xml_output=args.xml, tei_output=args.xmltei, validation=args.validate)
